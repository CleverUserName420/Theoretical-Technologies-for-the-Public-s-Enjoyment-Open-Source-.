#!/bin/bash
# OMG Cable Detection Suite v7.0 - M1 MacBook Active Scanner
...
echo -e "${CYAN}╔════════════════════════════════════════════════════════════════════════╗${NC}"
echo -e "${CYAN}║       OMG CABLE DETECTION SUITE v7.0 - COMPREHENSIVE SCANNER          ║${NC}"
echo -e "${CYAN}║   Cable Detection + WiFi/BT/Storage/Transmission/RF Analysis          ║${NC}"
echo -e "${CYAN}╚════════════════════════════════════════════════════════════════════════╝${NC}"
echo ""
...
analyze_cable_capabilities() {
    ...
    local KEY_EVENTS=$(log show --predicate 'subsystem == "com.apple.IOHIDFamily"' --last 5s 2>/dev/null | grep -c "Key" | awk '{print $1}')
...
main() {
    echo -e "${YELLOW}[*] OMG Cable Detection Suite v7.0${NC}"
    echo -e "${YELLOW}[*] Comprehensive Cable + Device Scanner${NC}"
    echo -e "${YELLOW}[*] Monitors: USB/WiFi/Bluetooth/Storage/Transmission/RF/Ultrasonic${NC}"
